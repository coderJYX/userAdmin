...
public class RedisUtil {
    public final static long DEFAULT_EXPIRE = 60 * 60 * 24; // 默认过期时长，单位：秒
    ...
    @Resource
    private ValueOperations<String, String> valueOperations;//支持string类型的value存储，set(key , value)

    @Resource
    private HashOperations<String, String, Object> hashOperations;//支持map类型的value存储，put(key, hashKep, hashValue)

    @Resource
    private ListOperations<String, Object> listOperations;//支持list类型的value存储，set(key, index, value)

    @Resource
    private SetOperations<String, Object> setOperations;//支持set类型的value存储，add(key, value1, value2, value3 ...)

    @Resource
    private ZSetOperations<String, Object> zSetOperations;//支持zSet类型的value存储，暂时未使用，有序集合类型


    /**
     * @param key 键
     * @return value 值
     * @Description:字符串的弹出操作【删除并返回】
     */
     ...
     * @param expire 续费
     */
    public void continueExpire(String key, long expire) {
        //设置过期时间
        if (expire != NOT_EXPIRE) {
            redisTemplate.expire(key, expire, TimeUnit.SECONDS);
        }
